async function drawMap() {
    ...
    //Accessor functions
    const countryNameAccessor = d => d.target.__data__.properties["NAME"];
    ...
    const [[x0, y0], [x2, y1]] = pathGenerator.bounds(sphere)
    ...
    const legenByline = legendGroup.append("text")
        .attr("y", -9)
    ...
    function onMouseEnter(datum) {
        ...
        const metricValue = metricDataByCountry[datum.target.__data__.properties.ADM0_A3]
        tooltip.select('#country')
            .text(countryNameAccessor(datum))
        tooltip.select('#value')
            .text(`${d3.format(",.2f")(metricValue || 0)}%`)

        const [centerX, centerY] = pathGenerator.centroid(datum.target.__data__)
